        ...
        <div class="justify-between mb-6 rounded-lg bg-white p-6 shadow-md sm:flex sm:justify-start" v-for="cart in getKeranjang" :key="cart.cart_id">
          ...
          <div class="sm:ml-4 sm:flex sm:w-full sm:justify-between">
            <div class="mt-5 sm:mt-0">
              ...
              <p class="mt-1 text-xs text-gray-700">stok :{{ cart.stock }}</p>
            </div>
            <div class="mt-4 flex justify-between sm:space-y-6 sm:mt-0 sm:block sm:space-x-6">
              <div class="flex items-center border-gray-100">
                <span class="cursor-pointer rounded-l bg-gray-100 py-1 px-3.5 duration-100 hover:bg-blue-500 hover:text-blue-50" @click="editKeranjang({cart_id :cart.cart_id, type :'minus'})"> - </span>
                <span class="h-8 w-8 border bg-white text-center text-xs outline-none" type="number" value="2" min="1 " > {{ cart.qty }}</span>
                <span class="cursor-pointer rounded-r bg-gray-100 py-1 px-3 duration-100 hover:bg-blue-500 hover:text-blue-50" @click="editKeranjang({cart_id :cart.cart_id, type :'plus'})"> + </span>
              </div>
              <div class="flex items-center space-x-4">
                <p class="text-sm">{{ cart.regular_price * cart.qty }}</p>
                <button @click="deleteKeranjang(cart.cart_id)">
